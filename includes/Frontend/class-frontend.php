...
final class Frontend {
	...
	private function render_button(
		...
		$bottom_offset = absint(
			$settings['bottom_offset'] ?? 24
		);

		$side_offset = absint(
			$settings['side_offset'] ?? 24
		);

		$z_index = absint(
			$settings['z_index'] ?? 999999
		);
	...
	private function get_contact_url(
		...
		switch ( $type ) {
			...
			case 'whatsapp':
				return 'https://wa.me/' . preg_replace(
					'/[^0-9]/',
					'',
					$value
				);
